...
const OFFERS = {
  ...
  monthly_169: {
    ...
    plan: "monthly_169",
    grantedCaseCount: 30,
    name: "Axe Flow - Offre Pro 30",
    unitAmount: 16900,
    priceId: "price_1UHgzYA5cbXOxF1tcN76SDnm",
  },
};
...
function normalizeOfferType(value) {
  ...
  if (["monthly_169", "subscription_169", "pro_30", "pro30", "abonnement_169", "169"].includes(v)) {
    return "monthly_169";
  }
...
function inferOfferFromSession(session) {
  ...
  if (mode === "subscription" && amountTotal === 16900) return "monthly_169";
...
async function createEntitlementFromCheckoutSession(session) {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (offerType === "monthly_89" || offerType === "monthly_169") {
      ...
      userUpdate.monthlyQuota = offer.grantedCaseCount;
